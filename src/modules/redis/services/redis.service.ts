...
export class RedisService {
  ...
  // srem забирає елемент з множини, якщо він є у ній, �� повертає кількість елементів, які були видалені
  ...
  // del забирає весь контент з множини, якщо він є у ній, повертає кількість елементів, які були видалені

  public async sMembers(key: string): Promise<string[]> {
    return await this.redisClient.smembers(key);
  }
  // sMember отримує всі елементи в множин, key: ключ множини. Повертає масив елементів, що містяться в множині

  public async expire(key: string, time: number): Promise<number> {
    return await this.redisClient.expire(key, time);
  }
  // встановлює таймаут на ключ, після якого ключ автоматично видаляється
